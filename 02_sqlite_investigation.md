...
HAVING parent.name like '%lasagne sheets%';
```

Note I had to do a `SELECT DISTINCT` on _both_ joins here because of the duplicates, which makes the query a bit slow, but it's tolerable (about 0.5s). 
This gives us _almost_ the right thing. The ingredients actually appear in the opposite order in the ingredients list. We could probably do something crazy like assign an autogenerated number to each child, then reverse the list before joining, but ~~I don't care enough~~ that's left as an exercise to the reader.
